...
{
    enum : u64 {
        FIXED_ARRAY_CUSTOM_SIZE=1 // Specify that there is a custom sizet specified by the void* to pack/unpack the array to
    };
}
...
template<class T>
typename std::enable_if<std::is_floating_point_v<T>, const char*>::type 
get_flag_for_type(T&var)
{
    return "%f";
}

template<class T>
typename std::enable_if<std::is_integral_v<T>, const char*>::type 
get_flag_for_type(T&var)
{
    return "%d";
}
