...
The network is set up so that any client can join the network when any one or more of the four devices are on. This is preferrable to the default Clearpath network setup as the Jackal and/or the Husky can operate the WiFi network without requiring any base stations. To accomplish this the routers are configured as access point repeaters with their DHCP servers handing out IP addresses from unique sets.
...
The Ubiquity Bullet M2 v.6.3.6 firmware file is saved as part of the project github repo \autocite{client_repo}.

** Router Configuration
Each host device on the network is configured with a static IP address on the subnet 192.168.10.0/24. The base station LAN ports are set to DHCP with a fallback IP in case there is no DHCP server. *Table [[table:ip_addresses]]* shows the device IP settings.
...
| *Device*       | *Port*           | *Address*                                   |
|----------------+------------------+---------------------------------------------|
| Base Station 1 | Wireless (WLAN0) | 192.168.10.1                                |
| Base Station 1 | Wired (LAN0)     | DHCP (192.168.1.20 fallback)                |
| Base Station 2 | Wireless (WLAN0) | 192.168.10.2                                |
| Base Station 2 | Wired (LAN0)     | DHCP (192.168.1.21 fallback)                |
| Jackal Router  | Wireless (WLAN0) | Bridged LAN0                                |
| Jackal Router  | Wired (LAN0)     | Bridged WLAN0 (192.168.10.3 for management) |
| Jackal CPU     | Wired (eth0)     | 192.168.10.5                                |
| Jackal LIDAR   | Wired            | 192.168.10.6                                |
| Husky Router   | Wireless (WLAN0) | Bridged LAN0                                |
| Husky Router   | Wired (LAN0)     | Bridged WLAN0 (192.168.10.4 for management) |
| Husky CPU      | Wired (eth0)     | 192.168.10.7 (bridged to eth1)              |
| Husky CPU      | Wired (eth1)     | 192.168.131.1 (bridged to eth0)             |
| Husky LIDAR    | Wired            | 192.168.131.201                             |
