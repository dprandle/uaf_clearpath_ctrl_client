...
The makePlan function, which must be implemented by every global planner do to it being a pure virtual function, takes a starting pose and goal and must fill in plan which is a reference to a vector of poses. The worst planner, for example, could just add the starting pose and goal pose to plan and return - this would likely get the robot stuck. The package global_planner provides implementations of A* and Dijkstra's path planning algorithms (as well as a few others), which are selectable via parameters. Navfn only provides A*.

** Simulation
Simulating the Jackal and Husky is easy as Clearpath authored packages which launch and set up ROS' simulation tool, Gazebo \autocite{gazebo}, with everything needed to simulate the robots. The simulation is somewhat limited with sensor nodes and data, but it does simulate laser scan data and driving which was enough to get the server/client app up and running. To simulate the Jackal and Husky, first install the correct packages:
...
The default jackal_world.launch creates the world shown in [[fig:jackal_world]]. The default world can be edited; its possible to create new geometry or load in models created by other Gazebo users. The model database contains tons of content, including complete office buildings! For this project, we stuck with the default world as the scope of the project didn't require anything else.

#+caption: Jackal default simulation world.
#+name:   fig:jackal_world
#+attr_latex: :width 6in
[[./images/overview_tf_small.png]]
\FloatBarrier
